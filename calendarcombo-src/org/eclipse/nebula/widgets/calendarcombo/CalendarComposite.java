...
class CalendarComposite extends Canvas implements MouseListener, MouseMoveListener
{
	...
	public CalendarComposite(Composite parent, Calendar selectedDay, Calendar disallowBeforeDate,
			Calendar disallowAfterDate, IColorManager colorManager, ISettings settings, boolean dateRange,
			Calendar rangeStart, Calendar rangeEnd)
	{
	...
	{
		Rectangle bounds = super.getBounds();
		Rectangle bgRect = new Rectangle(bounds.x + mSettings.getHeaderLeftMargin(), bounds.y
				+ mSettings.getHeaderTopMargin(), mSettings.getCalendarWidth() - 13, bounds.y
				+ mSettings.getHeaderHeight());
		gc.setBackground(mColorManager.getCalendarHeaderColor());
		gc.fillRectangle(bgRect);
		drawArrow(gc, bounds.x + mSettings.getHeaderLeftMargin() + mSettings.getArrowLeftSpacing() + 1, bounds.y
				+ mSettings.getHeaderTopMargin() + mSettings.getArrowTopSpacing() + 4, ARROW_LEFT);
		drawArrow(gc, bounds.x + mSettings.getCalendarWidth() - 13 - mSettings.getHeaderRightMargin(), bounds.y
				+ mSettings.getHeaderTopMargin() + mSettings.getArrowTopSpacing() + 4, ARROW_RIGHT);
		...
		mMonthNameBounds = new Rectangle(bounds.x + mSettings.getHeaderLeftMargin() + avail, bounds.y
				+ mSettings.getHeaderTopMargin() + 1, strWidth.x, strWidth.y);
		
		gc.drawString(toDraw, bounds.x + mSettings.getHeaderLeftMargin() + avail,
				bounds.y + mSettings.getHeaderTopMargin() + 1, true);
	}
	...
	private void drawTitleDays(GC gc)
	{
		...
		gc.drawLine(mSettings.getDatesLeftMargin() + 1, lineStart, bounds.width - mSettings.getDatesRightMargin() - 3,
				lineStart);
	...
	private void drawDays(GC gc)
	{
		...
		for (int y = 0; y < 42; y++)
		{
			...
			if (mSelectedDay != null)
			{
				if (DateHelper.sameDate(mSelectedDay, temp)
						|| (mDateRange && betweenDays != null && betweenDays.contains(temp)))
				{
					gc.setBackground(mColorManager.getSelectedDayColor());
					gc.fillRectangle(mDayXs[col] - mSettings.getOneDateBoxSize() - 4, mDatesTopY + spacer - 1,
							mSettings.getOneDateBoxSize() + 5, 14);
					gc.setBackground(mColorManager.getCalendarBackgroundColor());
				}
			}
			
			Rectangle dayBounds = new Rectangle(mDayXs[col] - mSettings.getOneDateBoxSize() - 4, mDatesTopY + spacer
					- 1, mSettings.getOneDateBoxSize() + 5, 14);
			...
			if (DateHelper.sameDate(mToday, temp))
			{
				...
				gc.drawRectangle(mDayXs[col] - mSettings.getOneDateBoxSize() - 4, mDatesTopY + spacer - 1,
						mSettings.getOneDateBoxSize() + 5, 14);
		...
		gc.setForeground(mColorManager.getLineColor());
		gc.drawLine(mSettings.getDatesLeftMargin() + 1, lastY, bounds.width - mSettings.getDatesRightMargin() - 3,
				lastY);
	}
	...
	class ButtonSectionLayout extends Layout
	{
		...
		protected void layout(Composite composite, boolean flushCache)
		{
			...
			if (mSettings.allowEmptyDate())
			{
				...
				children[0].setBounds(button1Left, vspacer, bwidth, bheight);
				children[1].setBounds(button2Left, vspacer, bwidth, bheight);
			}
			else
			{
				// see how much space we put on the left and right sides of the
				// buttons
				int width = mSettings.getCalendarWidth() - bwidth;
				width /= 2;
				
				children[0].setBounds(width, vspacer, bwidth, bheight);
			}
		}
	}
