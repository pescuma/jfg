...
public class CalendarCombo extends Composite
{
	...
	/**
	 * Creates a new calendar mCombo box with the given style and ISettings implementations.
	 * 
	 * @param parent Parent control
	 * @param style Combo style
	 ...
	 * @param colorManager IColorManager implementation
	 */
	public CalendarCombo(Composite parent, int style, CalendarCombo dependingCombo, ISettings settings,
					...
					else
					{
						...
						if (acceptedEvent)
						{
							...
							if (mStartDate != null)
							{
								...
								if (calType != -1)
								{
									Calendar newDate = (Calendar) mStartDate.clone();
									newDate.add(calType, up ? 1 : -1);
									setDate(newDate);
									...
									if (separatorChar != null)
									{
										...
										StringTokenizer st = new StringTokenizer(getText(), separatorChar);
										int count = 0;
										boolean stop = false;
										while (st.hasMoreTokens())
										{
	...
	private void parseTextDate()
	{
		...
		try
		{
			...
			if (comboText.length() < 1)
				...
				setDate(DateHelper.parse(comboText, mSettings.getLocale(), mSettings.getDateFormat(),
	...
	private void updateTextFromMStartDate()
	{
		String toSet;
		if (mStartDate == null)
			toSet = mSettings.getNoDateSetText();
		else
			toSet = DateHelper.getDate(mStartDate, DateHelper.dateFormatFix(mSettings.getDateFormat()));
	...
	public synchronized void setDateRange(Calendar start, Calendar end)
	{
		checkWidget();
		
		if (!mSettings.allowEmptyDate() && start == null)
			start = mSettings.getDefaultDate();
		if (!mAllowDateRange)
			end = null;
		start = toRange(start, mDisallowBeforeDate, mDisallowAfterDate);
		end = toRange(end, mDisallowBeforeDate, mDisallowAfterDate);
		
		boolean changedStart = areDifferent(start, mStartDate);
		boolean changedEnd = areDifferent(end, mEndDate);
		
		if (changedStart)
			mStartDate = copyAndFixDate(start);
		if (changedEnd)
			mEndDate = copyAndFixDate(end);
		
		// Always update text, because it may be out of sync with data
		updateTextFromMStartDate();
		
		if (changedStart)
			notifyDateChanged();
		
		if (changedEnd || (changedStart && mEndDate != null))
			notifyDateRangeChanged();
	}
	
	private Calendar toRange(Calendar date, Calendar start, Calendar end)
	{
		if (date == null)
			return null;
	...
	private Calendar copyAndFixDate(Calendar date)
	{
		if (date == null)
			return null;
	...
	private boolean areDifferent(Calendar d1, Calendar d2)
	{
		return d1 != d2 && (d1 == null || d2 == null || !DateHelper.sameDate(d1, d2));
	}
	
	public String getText()
	{
		checkWidget();
		return (isFlat ? mFlatCombo.getText() : mCombo.getText());
	}
	...
	public synchronized void showCalendar()
	{
		try
		{
			...
			mCalendarComposite = new CalendarComposite(mCalendarShell, pre, mDisallowBeforeDate, mDisallowAfterDate,
					mColorManager, mSettings, mAllowDateRange, mStartDate, mEndDate);
	...
	public boolean setFocus()
	{
		checkWidget();
		if (isFlat)
			return mFlatCombo.getTextControl().setFocus();
		else
			return mCombo.setFocus();
	}
	...
	public boolean forceFocus()
	{
		checkWidget();
		if (isFlat)
			return mFlatCombo.getTextControl().forceFocus();
		else
			return mCombo.forceFocus();
	}
	...
	{
		mDisallowBeforeDate = copyAndFixDate(disallowBeforeDate);
	...
	{
		mDisallowAfterDate = copyAndFixDate(disallowAfterDate);
		
		// Update current values
		if (mDisallowAfterDate != null)
			setDateRange(mStartDate, mEndDate);
	}
